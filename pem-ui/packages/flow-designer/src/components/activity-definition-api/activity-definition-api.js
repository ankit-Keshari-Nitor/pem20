...
export default function ActivityDefinitionAPI() {
  ...
  const fetchData = async () => {
    try {
      ...
      setRows(customizedData || []); // Safely set data or an empty array if content is undefined
    } catch (error) {
      console.error('Failed to fetch data:', error);

      setRows([]); // Reset data on error
    } finally {
    }
  };
  ...
  const filteredRows = rows.filter((row) => {
    ...
    if (filterKey && row[filterKey] !== undefined) {
      return row[filterKey].toString().toLowerCase().includes(searchLower);
    } else {
      return Object.keys(row).some((key) =>
        row[key] && row[key].toString().toLowerCase().includes(searchLower)
      );
    }
  });
  ...
  return (
    <div className="activities-list-container">
      <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'flex-end', marginBottom: '1rem' }}>
        <ExpandableSearch labelText="Search" placeholder="Search by Name/ Description/ ActivityDefnKey" onChange={(event) => setSearchQuery(event.target.value)} value={searchQuery} />
                ...
                <TableRow {...getRowProps({ row })}>
                  {row.cells.map((cell) => (
                    <TableCell key={cell.id}>
                      {cell.info.header === 'ellipsis' ? getEllipsis(cell.id) :
                        cell.value
                      }
                    </TableCell>
                  ))}
